from recommender.api import recommend, RecommendationRequest
...
def test_single_game_quota(mock):
    ...
    result = recommend(RecommendationRequest(rawg_ids=[1]))
...
def test_two_games_quota(mock):
    ...
    result = recommend(RecommendationRequest(rawg_ids=[1, 2]))
    rows = result["rawg_ids"]
    assert len(rows) == 3
    for row in rows:
        assert len(row) == 5
...
def test_three_games_quota(mock):
    ...
    result = recommend(RecommendationRequest(rawg_ids=[1, 2, 3]))
    rows = result["rawg_ids"]
    assert len(rows) == 3
    for row in rows:
        assert len(row) == 5
...
def test_caps_to_three_input_games(mock):
    ...
    result = recommend(RecommendationRequest(rawg_ids=[1, 2, 3, 4, 5]))
    rows = result["rawg_ids"]
    assert len(rows) == 3
